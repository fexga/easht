...
with open("energy_power_data_ray_workers.json") as f:
    data = json.load(f)
...
for series in result_list:
    ...
    for t in series["values"]:
        ts = float(t[0])
        watts = float(t[1])
        if watts > 0.16:  # Filter out 0 values
...
ax1.axvline(220/60, color='gray', linestyle=':', linewidth=2, label='Deploy/ Run')
ax1.axvline(1900/60, color='gray', linestyle=':', linewidth=2, label='Run/ Deprovision')
